<div x-data="{ show: <?php if ((object) ('isOpen') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('isOpen'->value()); ?>')<?php echo e('isOpen'->hasModifier('defer') ? '.defer' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('isOpen'); ?>')<?php endif; ?> }" x-show="show" x-transition:enter="transition ease-out duration-200"
    ...
    <div class="bg-white rounded-xl shadow-xl h-fit w-full max-w-3xl p-6 mt-5">
        ...
        <?php if($errorMessage): ?>
            <div class="alert alert-warning mb-4"><?php echo e($errorMessage); ?></div>
        <?php endif; ?>

        <?php if(count($visitHistory)): ?>
            <div class="my-6">
                ...
                <div class="overflow-x-auto">
                    <table class="table w-full table-zebra">
                        ...
                        <tbody>
                            <?php $__currentLoopData = $visitHistory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $visit): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td><?php echo e(\Carbon\Carbon::parse($visit['date'])->format('d/m/Y')); ?></td>
                                    <td><?php echo e(\Carbon\Carbon::parse($visit['time'])->format('H:i')); ?></td>
                                    <td class="text-wrap"><?php echo e($visit['observation'] ?? '—'); ?></td>
                                    <td><?php echo e($visit['tutor']); ?></td>
                                    <td class="text-center">
                                        <span
                                            class="badge badge-soft <?php echo e($visit['is_complete'] ? 'badge-success' : 'badge-error'); ?>">
                                            <?php echo e($visit['is_complete'] ? 'Sí' : 'No'); ?>
        ...
        <form wire:submit.prevent="save">
            ...
            <div class="mt-4 flex justify-end gap-2">
                ...
                <button type="submit" class="btn btn-primary"
                    <?php if(!$canBeRated): ?> disabled <?php endif; ?>>Guardar</button>
            </div>
        </form>
    </div>
    <div class="modal-backdrop" wire:click="closeModal"></div>
</div>
<?php /**PATH C:\laragon\www\practicasisus\resources\views/livewire/modals/final-grade-modal.blade.php ENDPATH**/ ?>