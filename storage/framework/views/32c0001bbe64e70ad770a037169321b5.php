...
<?php else: ?>
    <div class="w-full grid md:grid-cols-2 lg:grid-cols-3 place-items-center md:justify-center gap-8">
        <?php $__currentLoopData = $calls; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $call): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div wire:key="call-<?php echo e($call->id); ?>"
                class="card w-full sm:max-w-md py-12 px-5 hover:shadow-lg transition duration-300
                    <?php if($call->status === 'Activo'): ?> bg-green-50
                    <?php elseif($call->status === 'Finalizado'): ?> bg-red-50 <?php endif; ?>">
                ...
                <?php if (\Illuminate\Support\Facades\Blade::check('hasanyrole', ['admin', 'gestor-teacher'])): ?>
                    <div class="absolute right-2 top-2">
                        <div
                            ...
                            <button id="dropdown-toggle-<?php echo e($call->id); ?>" type="button"
                            ...
                            <div class="dropdown-menu dropdown-open:opacity-100 hidden p-0" role="menu"
                                aria-orientation="horizontal" aria-labelledby="dropdown-toggle-<?php echo e($call->id); ?>">
                                <div class="overflow-auto text-base-content/80 max-h-56 max-md:max-w-30">
                                    <div class="flex flex-col">
                                        <button wire:click="$emit('openEdit', <?php echo e($call->id); ?>)"
                                            class="dropdown-item inline-flex items-center gap-2">
                                            <span class="icon-[tabler--pencil] size-5"></span>
                                            <?php echo e(__('Editar')); ?>

                                        </button>
                                        <button wire:click="$emit('delete', <?php echo e($call->id); ?>)"
                                            class="dropdown-item inline-flex items-center gap-2">
                ...
                <div class="flex justify-center">
                    <figure
                        ...
                        <?php if($call->status === 'Activo'): ?> bg-green-100
                        <?php elseif($call->status === 'Finalizado'): ?> bg-red-100
                        <?php else: ?> bg-gray-100 <?php endif; ?>">
                        <span
                            class="icon-[tabler--speakerphone] size-16
                            <?php if($call->status === 'Activo'): ?> text-green-500/50
                            <?php elseif($call->status === 'Finalizado'): ?> text-red-600/70
                            <?php else: ?> text-gray-600 <?php endif; ?>"></span>
                    </figure>
                </div>
                ...
                <div class="flex justify-end mb-4">
                    <span
                        ...
                        <?php if($call->status === 'Activo'): ?> badge-success bg-green-600
                        <?php else: ?> badge-error <?php endif; ?>">
                        <span class="icon-[tabler--circle-check-filled] size-4"></span>
                        <?php echo e($call->status === 'Activo' ? __('Activo') : __('Finalizado')); ?>
                ...
                <div class="card-body">
                    <h5 class="card-title text-lg font-semibold mb-5"><?php echo e($call->name); ?></h5>
                    ...
                    <div class="card-footer">
                        <?php if($call->status == 'Activo'): ?>
                            ...
                            <button wire:click="$emit('openApplicationModal', <?php echo e($call->id); ?>)"
                                ...
                                <?php if($alreadyApplied): ?> disabled <?php endif; ?>>
                                <?php echo e($alreadyApplied ? 'Ya postulado' : 'Postularme'); ?>
...
} elseif ($_instance->childHasBeenRendered('l2820261239-0')) {
    $componentId = $_instance->getRenderedChildComponentId('l2820261239-0');
    $componentTag = $_instance->getRenderedChildComponentTagName('l2820261239-0');
    $html = \Livewire\Livewire::dummyMount($componentId, $componentTag);
    $_instance->preserveRenderedChild('l2820261239-0');
} else {
    $response = \Livewire\Livewire::mount('modals.application-modal');
    $html = $response->html();
    $_instance->logRenderedChild('l2820261239-0', $response->id(), \Livewire\Livewire::getRootElementTagName($html));
}
echo $html;
?>
                <?php endif; ?>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\practicasisus\resources\views/livewire/filters/call-card-filter.blade.php ENDPATH**/ ?>