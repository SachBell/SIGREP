...
@section('content')
    ...
    <div class="container-fluid d-flex flex-column gap-3">
        <div class="container-fluid">
            <form action="{{ route('dashboard.institutes.index') }}" method="GET" class="d-flex gap-2 align-items-center">
                <input type="text" name="search" placeholder="Buscar..." class="form-control"
                    value="{{ request()->query('search') }}" required>
                <button type="submit" class="btn btn-primary">Buscar</button>
                <a href="{{ route('dashboard.institutes.index') }}" class="btn btn-secondary">Resetear</a>
            </form>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="border border-gray-300">
                                    <form class="delete-form mb-2"
                                        action="{{ route('dashboard.institutes.destroy', $registro->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm delete-btn">Eliminar</button>
                                    </form>
                                    @include('components.alert-confirm')
                                    <a href="{{ route('dashboard.institutes.edit', $registro->id) }}"
    ...
    <div class="container-fluid">
        ...
        <div class="container-fluid">
            <a class="btn btn-primary" href="{{ route('institutes.create') }}">Añadir Institución</a>
        </div>
    </div>
@endsection
