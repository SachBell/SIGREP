@if ($userData)
    <form action="{{ route('user.profile.dataUpdate') }}" method="post" class="mt-6">
        @csrf
        @method('PUT')

        <div class="d-flex justify-content-between gap-4 px-0">
            <div class="container-fluid px-0">
                ...
                <div class="d-flex flex-column mb-2">
                    ...
                    <x-text-input id="cei" name="cei" type="text" class="mt-1 block w-full"
                        :value="old('cei', $userData->cei)" required autofocus autocomplete="cei" />
...
@else
    <form action="{{ route('user.profile.dataStore') }}" method="post" class="mt-6">
        @csrf
